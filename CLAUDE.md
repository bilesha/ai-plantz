...
QWEN_API_KEY=...
MOONSHOT_API_KEY=...
TREFLE_API_KEY=...           # Optional — powers GET /api/plant-search; free account at https://trefle.io/users/sign_up
PORT=5000
ALLOWED_ORIGIN=...          # CORS origin (defaults to http://localhost:8081)
MOCK_MODE=...                # Optional — "true" disables all AI provider calls and Trefle search; every endpoint returns a fixed mock response (used for automated test runs)
```
