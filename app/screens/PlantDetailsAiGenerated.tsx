...
export default function PlantDetailsAiGenerated() {
  ...
  return (
    ...
    >
      <Text className="text-3xl font-bold text-green-800 mb-4">{plantName}</Text>
      ...
      {!loading && !error && details && (
        <View className="my-4 p-4 bg-white rounded-lg shadow">
          {typeof details === "string" ? (
            // Fallback for when details is just a string    }
            <Text className="text-gray-800 text-base">{details}</Text>
          ) : (
            // Ideal rendering for a details object
            Object.entries(details).map(([key, value]) => (
              <View key={key}>
                <Text className="text-lg font-semibold capitalize text-green-700">
                  {key}
                </Text>
                <Text className="text-gray-800 text-base">{value as string}</Text>
              </View>
          ))
        )}
        </View>
